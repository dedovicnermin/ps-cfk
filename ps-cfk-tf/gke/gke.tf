...
resource "google_container_node_pool" "ora_nodes" {
  name           = "${var.owner_label}-ora"
  location       = var.region
  cluster        = google_container_cluster.gke_cluster.name
  node_count     = var.node_count_ora
  ...
  node_config {
    ...
    # preemptible  = true
    machine_type = var.node_machine_type_ora
    image_type   = "COS_CONTAINERD"
    disk_type    = "pd-balanced"
    disk_size_gb = var.node_disk_size_gb_ora
    taint        = [
      {
        effect = "NO_SCHEDULE"
        key    = "app"
